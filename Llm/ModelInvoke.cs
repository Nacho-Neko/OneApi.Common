...
///   <item><see cref="CallerFormat"/> — 调用方协议（<see cref="CallerFormats"/>）；</item>
///   <item><see cref="AccountUid"/> — 受信内网账户 id（替代 sk- 鉴权的 trust 值）。</item>
/// </list>
/// </summary>
public sealed record ModelInvoke(ModelRef Target, string CallerFormat, long AccountUid = 0)
{
    /// <summary>OpenAI Chat Completions 路径的便捷构造。</summary>
    public static ModelInvoke OpenAiChat(ModelRef target, long accountUid = 0) =>
        new(target, CallerFormats.OpenAiChat, accountUid);
